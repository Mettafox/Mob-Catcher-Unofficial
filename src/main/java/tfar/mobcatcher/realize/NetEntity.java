...
public class NetEntity extends ThrowableItemProjectile {
    ...
    private ItemStack getValidStack(ItemStack inputStack) {
        return inputStack.isEmpty() ? new ItemStack(ModItems.NET) : inputStack.copy();
    }
    ...
    public @NotNull ItemStack getItem() {
        if (this.stack.isEmpty()) {
            return new ItemStack(ModItems.NET);
        }
        return this.stack;
    }
    ...
    public void addAdditionalSaveData(@NotNull CompoundTag nbt) {
        ...
        if (!this.stack.isEmpty()) {
            nbt.put("mobcatcher", this.stack.save(level().registryAccess()));
        }
    }
    ...
    public void readAdditionalSaveData(@NotNull CompoundTag nbt) {
        ...
        if (nbt.contains("mobcatcher", Tag.TAG_COMPOUND)) {
            this.stack = ItemStack.parse(level().registryAccess(), nbt.getCompound("mobcatcher"))
                    .orElse(new ItemStack(ModItems.NET));
        } else {
            this.stack = new ItemStack(ModItems.NET);
        }
    }
